...
<div class="mb-4">
  <h5 class="fw-bold mb-1">
    <i class="fa-solid fa-handshake-angle text-success me-2"></i>Kampanye Bencana
  </h5>
  <p class="text-muted small mb-0">Daftar kampanye bencana yang sedang membutuhkan bantuan relawan.</p>
</div>
      ...
      <table id="tabel-kampanye" class="table table-hover mb-0 align-middle">
        ...
        <tbody>
          ...
          <tr data-nama="{{ strtolower($c->title) }}"
            ...
            <td class="pe-4">
              @if(in_array($c->id, $joinedCampaignIds))
                <button class="btn btn-sm btn-secondary rounded-pill px-3" disabled style="font-size: .8rem; font-weight: 600; white-space: nowrap; display: inline-flex; align-items: center; gap: 5px;">
                  <i class="fa-solid fa-circle-check"></i> Terdaftar
                </button>
              @else
                <a href="{{ route('relawan.volunteer-join.create', $c->id) }}" class="btn-detail">
                  <i class="fa-solid fa-handshake-angle"></i> Gabung Relawan
                </a>
              @endif
            </td>
          </tr>
