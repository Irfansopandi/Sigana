...
.avatar-placeholder {
  background: linear-gradient(135deg, #1e4f7e, #38bdf8);
  display: flex;
  align-items: center;
...
<div class="d-flex align-items-center gap-3 mb-1">
  <div class="rounded-3 p-2" style="background: rgba(30,79,126,0.1);">
    <i class="fa-solid fa-user fs-5" style="color:#1e4f7e;"></i>
  </div>
  <div>
...
<div class="row g-4">
  ...
  <div class="col-lg-8">
    ...
    <div class="card border-0 shadow-sm mb-4">
      <div class="card-body p-4">
        ...
        {{-- Form Edit Info --}}
        <form action="{{ route('relawan.profile.update-info') }}" method="POST">
          @csrf @method('PUT')
          <div class="row g-3">
            <div class="col-md-6">
    ...
    <div class="card border-0 shadow-sm">
      <div class="card-body p-4">
        ...
        @if(!$isGoogleOnly)
        <form action="{{ route('relawan.profile.update-password') }}" method="POST">
          @csrf @method('PUT')
        @endif
  ...
  {{-- Kanan: Status Akun (ala halaman Detail Relawan admin) --}}
  <div class="col-lg-4">
    <div class="card border-0 shadow-sm">
      <div class="card-body p-4">
        <div class="d-flex align-items-center gap-2 mb-4">
        ...
        <div class="mb-3">
          <div class="text-uppercase small text-muted fw-semibold mb-1">Verifikasi</div>
          @if($user->is_verified)
        ...
        <div class="mb-3">
          <div class="text-uppercase small text-muted fw-semibold mb-1">Kelengkapan Profil</div>
          @if($user->profile_complete)
            <span class="badge bg-success bg-opacity-10 text-success rounded-pill px-3 py-2">Lengkap</span>
          @else
            <span class="badge bg-warning bg-opacity-10 text-warning rounded-pill px-3 py-2">Belum Lengkap</span>
          @endif
        </div>

        <div class="mb-3">
          <div class="text-uppercase small text-muted fw-semibold mb-1">Tanggal Daftar</div>
          <div class="fw-medium">{{ $user->created_at->translatedFormat('d M Y, H:i') }}</div>
        </div>

        <div>
          <div class="text-uppercase small text-muted fw-semibold mb-1">Login Terakhir</div>
          <div class="fw-medium">
            {{ $user->last_login_at ? $user->last_login_at->translatedFormat('d M Y, H:i') : '-' }}
          </div>
        </div>
      </div>
    </div>
...
document.getElementById('photoInput').addEventListener('change', function () {
  ...
  fetch('{{ route("relawan.profile.update-photo") }}', {
    method: 'POST',
    body: formData,
  })
  .then(res => res.json())
    .then(data => {
        ...
        // Update foto di topbar & sidebar footer juga (tanpa reload)
        document.getElementById('topbarUserAvatar').innerHTML =
